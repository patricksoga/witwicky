...
class Model(nn.Module):
    ...
    def get_input(self, toks, is_src=True):
        ...
        if self.spd_centrality:
            word_embeds[:, 0, :] = word_embeds[:, 0, :] + self.centrality_embed(torch.LongTensor(0, device=torch.device('cuda')))

            word_embeds[:, -1, :] = word_embeds[:, -1, :] + self.centrality_embed(torch.LongTensor(0, device=torch.device('cuda')))

            word_embeds[:, 1:-1, :] = word_embeds[:, 0:-2, :] + self.centrality_embed(torch.LongTensor(1, device=torch.device('cuda')))
